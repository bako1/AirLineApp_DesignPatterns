...
public class MemberArchive {
    ...
    private HashMap<Integer, IBonusMember> bonusMemberMap;
    private IBonusMember basicMember;
    private SilverMember silverMember;

    public MemberArchive() {
        randomGenerator = new SecureRandom();
        bonusMemberMap = new HashMap<>();
        basicMember = new BasicMemberBuilder().setMemberNo(22).setPersonals(new PersonalsBuilder().setFirstName("Abdi").setSurname("Bako").setEmailAddress("" + "ab@gmail.com").setPassword("8976").createPersonals()).setEnrolledDate(LocalDate.now()).build();
    ...
    public int addMember(Personals personals, LocalDate dateEnrolled) {
        basicMember = new BasicMemberBuilder().setMemberNo(findAvailableNumber()).setPersonals(personals).setEnrolledDate(dateEnrolled).build();
     ...
     */
    public HashMap<Integer, IBonusMember> displayMembers() {
    ...
    public int registerPoints(int memberNo, int points) {

        IBonusMember bm = bonusMemberMap.get(memberNo);
        if (bm != null) bm.registerPoints(points);
        else return 0;


        return bm.getBonusPoints();
...
/*
    ...
    */
    ...
    public String toString() {
        return "MemberArchive " +bonusMemberMap.entrySet();
    }
}
